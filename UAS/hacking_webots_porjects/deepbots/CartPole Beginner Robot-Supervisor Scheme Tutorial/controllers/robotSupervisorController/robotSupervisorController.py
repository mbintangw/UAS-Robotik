...
class CartpoleRobot(RobotSupervisor):
    ...
    def get_observations(self):
        # Position on x axis
        cartPosition = normalizeToRange(self.robot.getPosition()[
                                        0], -0.4, 0.4, -1.0, 1.0)
        # Linear velocity on x axis
        cartVelocity = normalizeToRange(self.robot.getVelocity()[
                                        0], -0.2, 0.2, -1.0, 1.0, clip=True)
        # Pole angle off vertical
        poleAngle = normalizeToRange(
            self.positionSensor.getValue(), -0.23, 0.23, -1.0, 1.0, clip=True)
        # Angular velocity x of endpoint
        endpointVelocity = normalizeToRange(self.poleEndpoint.getVelocity()[
                                            3], -1.5, 1.5, -1.0, 1.0, clip=True)
    ...
    def is_done(self):
        ...
        cartPosition = round(self.robot.getPosition()[
                             0], 2)  # Position on x axis
    ...
    def solved(self):
        if len(self.episodeScoreList) > 100:  # Over 100 trials thus far
            # Last 100 episodes' scores average value
...
env = CartpoleRobot()
agent = PPOAgent(
    numberOfInputs=env.observation_space.shape[0], numberOfActorOutputs=env.action_space.n)
solved = False
episodeCount = 0
episodeLimit = 2000
...
while not solved and episodeCount < episodeLimit:
    ...
    for step in range(env.stepsPerEpisode):
        ...
        # observation for next step is current step's newObservation
